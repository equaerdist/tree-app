...
{
    public TreeNodeControllerTests(TestFixture fixture) : base(fixture)
    {
    }
    ...
    public async Task CreateNode_WhenNotUniqueAcrossSiblings_ShouldThrow()
    {
        ...
        //Act and Assert
        (await action.Should().ThrowAsync<ApiException>()).And.Message.Contains("Node with the same name already exists among siblings");
    }
    ...
    public async Task RenameNode_WhenAlreadyExistsTheSame_ShouldThrow()
    {
        ...
        //Прикрепимся к дереву сначала (первая нода в нем)
        var response = await TreeAppClient.CreateAsync(treeName, tree.Id, nodeName, CancellationToken);
        var firstChildResponse = await TreeAppClient.CreateAsync(treeName, response.Id, nodeName, CancellationToken);
        ...
        //Act and Assert
        (await action.Should().ThrowAsync<ApiException>()).And.Message.Contains("A sibling node with the same name already exists");
    }
    ...
    public async Task Delete_WhenNotExists_ShouldThrow()
    {
        ...
        //Act and Assert
        (await action.Should().ThrowAsync<ApiException>()).And.Message.Contains("Node not found in the specified tree");
    }
}